...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!description && archiveDetails) {
    try {
      const generated = await generateBookDescription(archiveDetails.title, archiveDetails.author || 'غير معروف', 'ar', seoBook?.category);
...
export default async function BookPage({ params }: Props) {
    ...
    if (extractedId) {
      ...
      if (archiveBook) {
        ...
        try {
          ...
          try {
            const generated = await generateBookDescription(archiveBook.title, archiveBook.author || 'غير معروف', 'ar', 'عام');
  ...
  if (!displayDescription && archiveBook) {
    try {
      const generated = await generateBookDescription(displayTitle, displayAuthor, 'ar', displayCategory);
