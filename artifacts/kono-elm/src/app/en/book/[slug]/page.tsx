...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!description && archiveDetails) {
    try {
      const generated = await generateBookDescription(archiveDetails.title, archiveDetails.author || 'Unknown', 'en', seoBook?.category);
...
export default async function EnglishBookPage({ params }: Props) {
    ...
    if (extractedId) {
      ...
      if (archiveBook) {
        ...
        try {
          ...
          try {
            const generated = await generateBookDescription(archiveBook.title, archiveBook.author || 'Unknown', 'en', 'General');
  ...
  if (!displayDescription && archiveBook) {
    try {
      const generated = await generateBookDescription(displayTitle, displayAuthor, 'en', displayCategory);
